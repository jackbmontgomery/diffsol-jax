...
def test_lv_matches_scipy(method):
    ...
    assert diff < 1e-3, f"{method}: max diff {diff:.2e}"


@pytest.mark.parametrize("method", METHODS)
def test_lv_grad_matches_fd(method):
    solver, _ = make_diffsol_solver(
        lotka_volterra,
        y0=Y0,
        p_example=PARAMS,
    ...
    assert rel < 1e-2, f"{method}: grad rel error {rel:.2e}"


def test_unknown_method_raises():
    with pytest.raises(ValueError, match="unknown method"):
        make_diffsol_solver(
            lotka_volterra, y0=Y0, p_example=PARAMS, method="rk4_custom"
        )
